...
export function SiteHeader() {
  ...
  return (
    ...
    >
      ...
      {/* Main Navigation */}
      <div className="container mx-auto px-3 sm:px-4 md:px-6 my-0">
        {/* Grid Container */}
        <div className="grid grid-cols-[auto_1fr] gap-2 sm:gap-4">
          ...
          <div className="flex items-center">
            <a href="/" className="flex items-center">
              <Image
                src={logoSrc}
                alt="GLA logo"
                className="h-16 sm:h-20 md:h-24 mt-2 sm:mt-4 md:mt-0 w-auto"
                width={200}
                height={96}
              />
            </a>
          ...
          <div className="flex items-center justify-end">
            ...
            <div className="hidden lg:flex items-center space-x-2 xl:space-x-4">
              <nav className="flex items-center space-x-0.5 xl:space-x-1 flex-wrap">
                {NAV_ITEMS.map((item) =>
                  item.dropdownItems ? (
                    <div key={item.path} className="relative group">
                      <button
                        className={`flex items-center px-2 xl:px-4 py-2 text-xs xl:text-sm font-medium rounded-md transition-colors hover:bg-cusYellow whitespace-nowrap ${
                          scrolled
                            ? "text-black"
                            : isProgramPage ||
                              isContactPage ||
                      ...
                      <div className="absolute left-0 top-full w-48 mt-2 bg-white rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                        <div className="p-2">
                          {item.dropdownItems.map((dropdownItem) => (
                            <Link
                              key={dropdownItem.path}
                              href={dropdownItem.path}
                              className="block px-4 py-2 text-sm text-black bg-white hover:bg-yellow-300 rounded-md transition-colors"
                            >
                              {dropdownItem.title}
                            </Link>
                          ))}
                    ...
                    <Link
                      key={item.path}
                      href={item.path}
                      className={`px-2 xl:px-4 py-2 text-xs xl:text-sm font-medium rounded-md transition-colors hover:bg-cusYellow whitespace-nowrap ${
                        scrolled
                          ? "text-black"
                          : isProgramPage ||
                            isContactPage ||
...
function MobileNav({ scrolled, isScrollingUp }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+0.5rem)] w-[calc(100vw-2rem)] sm:w-72 max-w-sm origin-top-right rounded-md border bg-white shadow-lg text-black z-50 max-h-[calc(100vh-5rem)] overflow-y-auto">
          <div className="p-2">
            {NAV_ITEMS.map((item) =>
              item.dropdownItems ? (
                <MobileNavItem
                ...
                <Link
                  key={item.path}
                  href={item.path}
                  className="block rounded-md px-3 py-2 text-sm font-medium text-cusBlue bg-white hover:bg-yellow-300 transition-colors"
...
function MobileNavItem({ title, items, setOpen }) {
  ...
  return (
    <div>
      <button
        className="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm font-medium bg-white hover:bg-yellow-300 transition-colors"
        ...
        <div className="ml-4 mt-1 space-y-1">
          {items.map((item) => (
            <Link
              key={item.path}
              href={item.path}
              className="block rounded-md px-3 py-2 text-sm font-medium bg-white hover:bg-yellow-300 transition-colors text-black"
              onClick={() => setOpen(false)}
            >
              {item.title}
            </Link>
